...
- **2モードの記録を分離**:
  - 「相手の攻撃編成を記録」— 自分が防衛側で、相手に攻撃されたときの相手編成(このツールの本来の用途)。ポジション表記は A1〜A4 / SP1〜SP2
  - 「相手の防衛編成を記録」— 自分が攻撃側で、相手の防衛編成を攻略したときの記録。ポジション表記は D1〜D4 / SP1〜SP2
  - 使用キャラ集計・ポジション別集計・相手別遭遇回数・保存済み履歴は、選択中のモードのデータだけに絞って表示される
  - 旧データ(モード分割前に保存したもの)は「相手の攻撃編成」扱いになる(このツールの元々の用途に合わせた既定値)
- **自分の編成欄**: 対戦相手とは無関係に、自分が今使っている防衛編成(D1〜D4/SP1〜SP2)・攻撃編成(A1〜A4/SP1〜SP2)を1件だけ保存できるメモ欄
...
- 6枠入力、役割(ストライカー/スペシャル)の食い違い警告つき
...
- 旧ローカル版(`/Users/kiyui/defense-comp-tracker/data.json`)のデータをこちらに移したい場合:
  ...
  3. 「エクスポート」でダウンロードしたJSONは`{records, myTeams}`形式で、インポート時に自分の編成欄も一緒に復元される
